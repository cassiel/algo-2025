...
(->>
 (reset! SEQ {:sequences {:main {1 [[0 :Basic :note (+ 48 2) 40 100]]
                                 #_ [[0 :Basic :note 60 64 100]
                                     [0.5 :Basic :note 60 40 100]]
                                 3 [[0 :Basic :note (+ 48 2 ) 20 100]
                                    [0.5 :Basic :note (+ 60 2) 40 100]]
                                 4 [[0 :Basic :note (+ 48 2) 20 100]]}

                          :FFF {1 [[0 :Filter_MINI :param :Cutoff 0.5]
                                   [0.5 :Filter_MINI :param :Cutoff 0.0]]
                                2 [[0 :Filter_MINI :param :Cutoff 0.5]
                                   [0.5 :Filter_MINI :param :Cutoff 0.0]]
                                3 [[0 :Filter_MINI :param :Cutoff 0.4]]
...
(->>
 (reset! SEQ {:sequences {:FFF {1 [[0 :Filter_MINI :param :Cutoff 0.3]]
                                2 [[0 :Filter_MINI :param :Cutoff 0.8]
                                   [0.5 :Filter_MINI :param :Cutoff 0.0]]
                                3 [[0 :Filter_MINI :param :Cutoff 0.4]]
                                4 [[0 :Filter_MINI :param :Cutoff 0.45]]}
                          :GGG {1 [[0 :A :B :C]]}}
...
(reset! PARAMS nil)
;; DANGEROUS: (deref PARAMS)

(deref PARAMS)

(px/get-matching PARAMS :Basic #"LFO")
(px/get-matching PARAMS :Enso #"Link")
(px/get-matching-to-dict PARAMS :Enso #"Mode")

(px/get-matching-to-dict PARAMS :Basic #"LFO")
(px/get-matching PARAMS :Enso #"Speed")
(px/get-matching PARAMS :Replika_XT #"Mix")
(px/get-matching PARAMS :Filter_MINI #"Cutoff")
(px/get-matching-to-dict PARAMS :Filter_MINI #"Cutoff")
...
;; Bring in Enso:
(ctrl/mix :Enso :Enso -20
          (ctrl/mix :IO :Enso 0 10))
(ctrl/mix :Enso :IO 0 10)

(ctrl/mix :IO :Enso -40 4)
(ctrl/mix :Enso :IO -40 2)

(ctrl/mix :Axon :IO -40 2)


ctrl/mix (:* :* -40 5)

(ctrl/mix :Filter_MINI :Filter_MINI -40 1)
...
(px/get-matching PARAMS :Filter_MINI #"Res")
(px/get-matching PARAMS :Filter_MINI #"Cutoff")
(px/get-matching PARAMS :Filter_MINI #"Volume")

(px/xmit-some-params-now :Filter_MINI
                         [:Cutoff 0.7]
                         [:Resonance 0.4]
                         [:Output_Volume 0.7])
...
(swap! SEQ
       {:sequences {} :messages nil})

(deref SEQ)

(ctrl/mix :Basic :IO -40 5)

(ctrl/mix :IO :Filter_MINI -40)
(ctrl/mix :IO :IO -10)

(swap! SEQ assoc-in [:sequences :FFF] {1 [[0 :Filter_MINI :param :Cutoff 0.4]]
                                       2 [[0 :Filter_MINI :param :Cutoff 0.6]
                                          [0.5 :Filter_MINI :param :Cutoff 0.0]]
                                       3 [[0 :Filter_MINI :param :Cutoff 0.2]]
                                       })

(ctrl/mix :IO :Filter_MINI 0)
(ctrl/mix :Filter_MINI :IO 0)
...
(swap! SEQ assoc-in [:sequences :FFF] {1 [[0 :Filter_MINI :param :Cutoff 0.2]]
                                       2 [[0 :Filter_MINI :param :Cutoff 0.9]
                                          [0.5 :Filter_MINI :param :Cutoff 0.1]]
                                       3 [[0 :Filter_MINI :param :Cutoff 0.2]]
                                       4 [[0 :Filter_MINI :param :Cutoff 0.3]]})

(px/get-matching PARAMS :Filter_MINI #"Output")
(px/xmit-some-params-now :Filter_MINI [:Output_Volume 0.8])
...
;; After a bit: Basic into :Filter_MINI
(ctrl/mix :Basic :Filter_MINI -40)
...
(ctrl/window :Filter_MINI 1)
(ctrl/window :Replika_XT 1)

;; Sitar -> Replika
(ctrl/mix :IO :Replika_XT 0)
;; Replika -> Filter_MINI
(ctrl/mix :Replika_XT :Filter_MINI 0)
;; Filter_MINI -> Out
(ctrl/mix :Filter_MINI :IO 6)

;; Set up Filter_MINI
(go (<! (ctrl/reset-device :Filter_MINI))
    (<! (px/request-params PARAMS :Filter_MINI)))

;; Filter setup
(px/xmit-some-params-now :Filter_MINI [:Resonance 0.8])
